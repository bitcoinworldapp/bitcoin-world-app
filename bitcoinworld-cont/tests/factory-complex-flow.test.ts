...
// Debe coincidir con la constante ADMIN del contrato
...
describe("Market Factory v2 - complex suite (3 markets, quotes, slippage, fee lock, add-liquidity, resolves, redemptions)", () => {
  it("runs 3 markets in parallel with mixed actions and verifies balances, quotes, and final drains", () => {
    ...
    // Seed balances (mint lo firma el owner del token: deployer)
    expect(mint(ADMIN, 200_000, d).result.type).toBe("ok"); // liquidez inicial saldrá del ADMIN
    ...
    expect(simnet.callPublicFn("market-factory-v2","set-fee-recipients",[toP(DRP), toP(BRC), toP(TM), toP(LP)], ADMIN).result.type).toBe("ok");

    const M0 = 10, M1 = 11, M2 = 12;
    console.log(`[CREATE] M0=${M0} M1=${M1} M2=${M2}`);
    expect(simnet.callPublicFn("market-factory-v2","create-market",[toU(M0), toU(40_000)], ADMIN).result.type).toBe("ok");
    expect(simnet.callPublicFn("market-factory-v2","create-market",[toU(M1), toU(60_000)], ADMIN).result.type).toBe("ok");
    expect(simnet.callPublicFn("market-factory-v2","create-market",[toU(M2), toU(80_000)], ADMIN).result.type).toBe("ok");
    ...
    const qBefore = unwrapQuote(simnet.callReadOnlyFn("market-factory-v2","quote-buy-yes",[toU(M0), toU(200)], d).result);
    ...
    expect(simnet.callPublicFn("market-factory-v2","buy-yes-auto",[toU(M0), toU(150), toU(1_000_000), toU(100_000)], w1).result.type).toBe("ok");
    expect(simnet.callPublicFn("market-factory-v2","buy-no-auto", [toU(M0), toU(300), toU(1_000_000), toU(100_000)], w2).result.type).toBe("ok");
    ...
    expect(simnet.callPublicFn("market-factory-v2","add-liquidity",[toU(M0), toU(10_000)], ADMIN).result.type).toBe("ok");
    const qAfter = unwrapQuote(simnet.callReadOnlyFn("market-factory-v2","quote-buy-yes",[toU(M0), toU(200)], d).result);
    ...
    const slFail = simnet.callPublicFn("market-factory-v2","buy-yes-auto",[toU(M1), toU(100), toU(1_000_000), toU(1)], w3).result;
    expect(slFail).toEqual({ type:"err", value:{ type:"uint", value: 732n } });

    console.log("[BUY] M1: valid YES x120 (w3), valid NO x150 (w4)");
    expect(simnet.callPublicFn("market-factory-v2","buy-yes-auto",[toU(M1), toU(120), toU(1_000_000), toU(100_000)], w3).result.type).toBe("ok");
    expect(simnet.callPublicFn("market-factory-v2","buy-no-auto", [toU(M1), toU(150), toU(1_000_000), toU(100_000)], w4).result.type).toBe("ok");
    ...
    const capFail = simnet.callPublicFn("market-factory-v2","buy-yes",[toU(M2), toU(50)], w1).result;
    expect(capFail).toEqual({ type:"err", value:{ type:"uint", value: 730n } });

    console.log("[BUY] M2: auto buys with bump-cap");
    expect(simnet.callPublicFn("market-factory-v2","buy-yes-auto",[toU(M2), toU(200), toU(50_000), toU(100_000)], w1).result.type).toBe("ok");
    expect(simnet.callPublicFn("market-factory-v2","buy-no-auto", [toU(M2), toU(250), toU(50_000), toU(100_000)], w2).result.type).toBe("ok");
    ...
    expect(simnet.callPublicFn("market-factory-v2","lock-fees-config",[], ADMIN).result.type).toBe("ok");
    const changeRec = simnet.callPublicFn("market-factory-v2","set-fee-recipients",[toP(w1), toP(w2), toP(w3), toP(w4)], ADMIN).result;
    const changeFee = simnet.callPublicFn("market-factory-v2","set-fees",[toU(200), toU(50)], ADMIN).result;
    ...
    expect(simnet.callPublicFn("market-factory-v2","resolve",[toU(M2), Cl.stringAscii("NO")], ADMIN).result.type).toBe("ok");

    console.log("[REDEEM] winners on each market");
    expect(simnet.callPublicFn("market-factory-v2","redeem",[toU(M0)], w2).result.type).toBe("ok"); // NO winner
    expect(simnet.callPublicFn("market-factory-v2","redeem",[toU(M1)], w3).result.type).toBe("ok"); // YES winner
    expect(simnet.callPublicFn("market-factory-v2","redeem",[toU(M2)], w2).result.type).toBe("ok"); // NO winner

    // drain leftovers
    simnet.callPublicFn("market-factory-v2","redeem",[toU(M0)], w1);
    simnet.callPublicFn("market-factory-v2","redeem",[toU(M1)], w4);
    simnet.callPublicFn("market-factory-v2","redeem",[toU(M2)], w1);
