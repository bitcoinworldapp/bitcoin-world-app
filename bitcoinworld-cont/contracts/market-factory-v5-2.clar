;; ------------------------------------------------------------
;; market-factory-v5-2 (Fixed-b LMSR + fixed-per-share redemption)
...
;; - NEW: ensure-user-balance pre-check + clearer error for insufficient funds,
;;        and lightweight debug prints around buy paths.
;; ------------------------------------------------------------
...
;; NEW explicit errors for wallet balance path
(define-constant ERR-NO-WALLET-BAL (err u760))  ;; user does not have enough sBTC for TOTAL (base + fees)
...
;; NEW: read balance and assert it covers `need`
  ...
  (begin
    ...
    (let (
      ...
      (let (
        ...
        ;; NEW: explicit wallet-balance check before any transfer
